...
{
    int  pos = hash_string(item, this->size);
    struct listset *tmp = this->table[pos];
    return  listset_lookup(tmp, item);
}
...
{
    int index = hash_string(item, this->size);
    struct  listset *temp = this->table[index];
    if(listset_lookup(temp, item))
    {
        listset_remove(temp, item);
    }
