...
{
    int index1,index2;
    index1 = hash_string(item, BLOOM_HASH1, this->size);
    int found = bitset_lookup(this->bitset, index1);
    index2 = hash_string(item, BLOOM_HASH2, this->size);
    return found && bitset_lookup(this->bitset, index2);
}
...
void bloom_add(struct bloom * this, char * item) {
    ...
    int found = bitset_lookup(this->bitset, index1);
    index2 = hash_string(item, BLOOM_HASH2, this->size);
    bitset_add(this->bitset, index1);
    bitset_add(this->bitset, index2);
}
